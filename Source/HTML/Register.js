...
RegisterUsernameInput.onblur = () => {
    ...
    else if (!Regexes.Username.test(RegisterUsernameInput.value)) {
...
RegisterNicknameInput.onblur = () => {
    if (RegisterNicknameInput.value.length === 0) {
        SetValid(RegisterNicknameInput);
    }
    else if (!Regexes.Nickname.test(RegisterNicknameInput.value)) {
...
RegisterPasswordInput.onblur = () => {
    ...
    else if (Regexes.Password.test(RegisterPasswordInput.value)) {
...
RegisterPasswordRepeatInput.onblur = () => {
    if (RegisterPasswordRepeatInput.value.length === 0) {
        SetValid(RegisterPasswordRepeatInput);
    }
    else if (RegisterPasswordRepeatInput.value != RegisterPasswordInput.value) {
...
RegisterEmailInput.onblur = () => {
    ...
    else if (!Regexes.EmailAddress.test(RegisterEmailInput.value)) {
...
RegisterEmailVerificationCodeInput.onblur = () => {
    if (RegisterEmailVerificationCodeInput.value.length === 0) {
        SetValid(RegisterEmailVerificationCodeInput);
    }
    else if (!Regexes.EmailVerificationCode.test(RegisterEmailVerificationCodeInput.value)) {
